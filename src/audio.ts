/**
 * Audio generator using the Web Audio API for tactical castle battle sound effects
 */
...
class SoundEngine {
  ...
  // Play building placement / purchase sound
  public playPlace() {
  ...
  // Battle War Horn (When round starts or Maru falls)
  public playWarHorn() {
    if (this.isMuted) return;
    this.init();
    if (!this.ctx) return;
    ...
    osc.frequency.exponentialRampToValueAtTime(330, this.ctx.currentTime + 0.6);

    gain.gain.setValueAtTime(0.01, this.ctx.currentTime);
    gain.gain.linearRampToValueAtTime(0.3, this.ctx.currentTime + 0.1);
    gain.gain.setValueAtTime(0.3, this.ctx.currentTime + 0.5);
  ...
  // CPU progressive building hammer sound
  public playBuildHammer() {
    if (this.isMuted) return;
    this.init();
    if (!this.ctx) return;
  ...
  // Rally Beacon Call (War Fanfare)
  public playRallyOrder() {
    if (this.isMuted) return;
    this.init();
    if (!this.ctx) return;
  ...
  public playPathDraw() {
    ...
    osc.frequency.exponentialRampToValueAtTime(780, this.ctx.currentTime + 0.09);
